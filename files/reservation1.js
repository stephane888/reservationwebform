  jQuery(document).ready(function($) 
 {
    ...
    function recapitulatif() {
    	...
    	setTimeout(function () {
    		if ($datedepart.val() && $datearriver.val()  ) {
    				...
    				$('#recapitulatif .option').html('<strong> Petit-déjeuner : </strong> <span class="price">'+ nbreAdultes*nombrejour*5 +'</span> €');
    				checktotal();
    		}
    	}, 150);
    	//// affiche les opions
    ...
    function checkoptions() {
		...
		for( i=0; i<atLeastOneIsChecked; i++ ){ if(checkedtable[i]>0){checked = checkedtable[i] + checked;} }
    		...
    		var valeur3 = $('#edit-submitted-adultes').val();
    		if (valeur3>1) {nbreAdultes=valeur3;}else {nbreAdultes=1;}
    		$('#recapitulatif .option').html('<strong> Petit-déjeuner : </strong> <span class="price">'+ nbreAdultes*checked*5 +'</span> €');
    		checktotal();
    }
